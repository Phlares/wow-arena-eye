...
const tracks: GoTrack[] = [
  { unitId: 'F1', name: 'Me', team: 'friendly', className: 'Warlock', intervals: [{ startSec: 10, endSec: 20, spell: 'Dark Soul: Misery' }] },
...
it('hovers the ability used on a GO segment', () => {
  const { container } = render(<GoTracks tracks={tracks} matchEnd={60} />);
  const seg = [...container.querySelectorAll('.go-seg')].find((s) => s.getAttribute('title')?.includes('Me'));
  expect(seg?.getAttribute('title')).toMatch(/Dark Soul: Misery/);  // the ability that opened the go
});
