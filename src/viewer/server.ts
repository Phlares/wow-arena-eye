...
export function handleApi(db: DatabaseSync, method: string, path: string, params: URLSearchParams, gapMs: number): ApiResult {
  ...
  if (detail) {
    ...
    return metrics ? json(200, { metrics, rangeSeries: buildRangeSeries(metrics), rangeTargets: buildRangeTargets(metrics), roster: buildRoster(metrics), goTracks: buildGoTracks(metrics) }) : json(404, { error: 'no detail for match (re-ingest to populate)' });
  }
