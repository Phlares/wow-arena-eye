 ...
 *  1. The player itself.
 *  2. Pets summoned in-window (SPELL_SUMMON with player as source) — cheap fallback
 *     for pets summoned during the match window.
 *  3. Units in match.units whose ownerId === player — the authoritative path; catches
 *     pre-summoned pets (e.g. a warlock Felhunter with a randomized name like "Zhaazhem")
 *     that never emit a SPELL_SUMMON inside the log window. Players themselves have
 *     ownerId === "0", so they are not falsely included here.
 *
